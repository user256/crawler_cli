...
artifacts — and the ``crawler-cli/crawl-artifact/9`` envelope that carries it.
...
def test_manifest_backed_artifact_declares_the_current_schema_version() -> None:
    ...
    payload = serialize_crawl_job(job)
    assert payload["schema_version"] == CRAWL_ARTIFACT_SCHEMA_VERSION == "crawler-cli/crawl-artifact/9"
