...
|---|---|---|
| `crawler-cli/crawl-artifact/5` | saved crawl JSON (`serialize_crawl_job`), including body accounting, typed budget terminal state, static JS/CSS candidates, render-time URL evidence, and the `authorization_scope` record (null when no scope manifest was used) | `tests/contract/golden/crawl_artifact.json` |
