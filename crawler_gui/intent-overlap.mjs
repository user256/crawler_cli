...
export function createViewerState(model) {
  return {
    search: "", risk: "all", activeTab: "pages", selectedIndex: null,
    types: Object.fromEntries(TYPE_FILTERS.map(([id, , enabled]) => [id, enabled])),
    clusters: Object.fromEntries(model.clusters.map((cluster) => [String(cluster.id), true])),
    allClusters: true, sort: { pages: ["max_similarity", -1], pairs: ["similarity", -1], clusters: ["size", -1] },
  };
}
...
export function pageVisible(page, state) {
  ...
  if (!state.allClusters && (!page.cluster_id || !state.clusters[String(page.cluster_id)])) return false;
...
export function mountIntentOverlapViewer(root, model) {
  ...
  root.innerHTML = `<section class="intent-view" aria-label="Intent Overlap report">
    ...
    <div class="intent-layout"><aside class="intent-filters"><label>URL search <input id="intent-search" type="search" placeholder="Filter URLs…"></label><label>Risk <select id="intent-risk"><option value="all">All risk</option><option value="duplicate">Duplicate</option><option value="overlap">High overlap + duplicate</option></select></label><h3>Match / page types</h3><div id="intent-types"></div><h3>Clusters</h3><label><input id="intent-all-clusters" type="checkbox" checked> All clusters</label><div id="intent-clusters"></div></aside>
    <main class="intent-main"><div class="intent-map-wrap"><canvas id="intent-map" aria-label="Cluster map"></canvas><div id="intent-tooltip" class="intent-tooltip"></div><small>Wheel zoom · drag pan · click to pin · hover highlights cluster mates</small></div><section class="intent-tables"><nav id="intent-tabs" class="detail-tabs"><button data-tab="pages" class="active">Pages</button><button data-tab="pairs">Pairs</button><button data-tab="clusters">Clusters</button></nav><div id="intent-table" class="table-wrap"></div><div id="intent-detail" class="intent-detail">Select a map point or table row to inspect it.</div></section></main></div></section>`;
  root.querySelector("#intent-run").textContent = `Run ${model.run.id} · ${model.run.label} · ${model.run.completedAt} · ${model.run.source}`;
  const exportLink = root.querySelector("#intent-export"); exportLink.href = model.run.artifact; exportLink.textContent = "Export HTML (offline artifact)";
  ...
  const view = { scale: 1, tx: 0, ty: 0, dragging: false, x: 0, y: 0, hover: null };
  const color = (id) => `hsl(${[...String(id || "none")].reduce((n, c) => n * 31 + c.charCodeAt(0), 7) % 360} 62% 52%)`;
  const point = (page) => { const w = canvas.clientWidth, h = canvas.clientHeight, pad = 24; return { x: (pad + ((page.coords[0] - bounds.minX) / (bounds.maxX - bounds.minX)) * (w - pad * 2)) * view.scale + view.tx, y: (pad + (1 - (page.coords[1] - bounds.minY) / (bounds.maxY - bounds.minY)) * (h - pad * 2)) * view.scale + view.ty }; };
  ...
  const resize = () => { const rect = canvas.parentElement.getBoundingClientRect(), dpr = devicePixelRatio || 1; canvas.width = rect.width * dpr; canvas.height = Math.max(280, rect.height) * dpr; canvas.style.width = `${rect.width}px`; canvas.style.height = `${Math.max(280, rect.height)}px`; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); draw(); };
  ...
  const render = (focusSelected = false) => { const summary = model.raw.summary || {}; root.querySelector("#intent-summary").innerHTML = [["embedded", summary.embedded], ["pairs", summary.overlap_pairs], ["clusters", summary.clusters], ["duplicates", summary.duplicate_pages], ["thin", summary.thin_content_pages], ["threshold", summary.threshold]].map(([k, v]) => `<span>${k}: <strong>${esc(v ?? "—")}</strong></span>`).join("");
    ...
    root.querySelector("#intent-clusters").innerHTML = model.clusters.map((cluster) => `<label><input type="checkbox" data-cluster="${esc(cluster.id)}" ${state.clusters[String(cluster.id)] ? "checked" : ""}> ${esc(cluster.label || cluster.id)} (${cluster.size})</label>`).join("");
    const rows = state.activeTab === "pages" ? sorted(model.pages.filter((page) => pageVisible(page, state)), state.sort.pages) : state.activeTab === "pairs" ? sorted(model.pairs.filter((pair) => pairVisible(pair, state, model)), state.sort.pairs) : sorted(model.clusters.filter((cluster) => state.allClusters || state.clusters[String(cluster.id)]), state.sort.clusters);
    ...
    const rerender = (shouldFocus = false) => { render(shouldFocus); }; rerender.clusterFirst = (id) => selectCluster(state, model, id); table(root.querySelector("#intent-table"), columns, rows, state.activeTab, state, rerender);
    root.querySelectorAll("[data-type]").forEach((input) => input.onchange = () => { state.types[input.dataset.type] = input.checked; render(); }); root.querySelectorAll("[data-cluster]").forEach((input) => input.onchange = () => { state.clusters[input.dataset.cluster] = input.checked; state.allClusters = false; root.querySelector("#intent-all-clusters").checked = false; render(); });
  ...
  root.querySelector("#intent-search").oninput = (e) => { state.search = e.target.value; render(); }; root.querySelector("#intent-risk").onchange = (e) => { state.risk = e.target.value; render(); }; root.querySelector("#intent-all-clusters").onchange = (e) => { state.allClusters = e.target.checked; if (state.allClusters) Object.keys(state.clusters).forEach((id) => { state.clusters[id] = true; }); render(); }; root.querySelector("#intent-tabs").onclick = (e) => { const button = e.target.closest("[data-tab]"); if (!button) return; state.activeTab = button.dataset.tab; root.querySelectorAll("[data-tab]").forEach((tab) => tab.classList.toggle("active", tab === button)); render(); };
  canvas.onmousemove = (e) => { const r = canvas.getBoundingClientRect(), x = e.clientX - r.left, y = e.clientY - r.top; if (view.dragging) { view.tx += x - view.x; view.ty += y - view.y; view.x = x; view.y = y; draw(); return; } const page = hit(x, y); view.hover = page?._index ?? null; if (page) { tip.hidden = false; tip.style.left = `${x + 12}px`; tip.style.top = `${y + 12}px`; tip.innerHTML = `<code>${esc(page.url)}</code><br>${esc(model.clusterById.get(String(page.cluster_id))?.label || page.cluster_id || "unclustered")} · ${esc(page.risk || "no risk")}`; } else tip.hidden = true; draw(); }; canvas.onmouseleave = () => { view.hover = null; tip.hidden = true; draw(); }; canvas.onmousedown = (e) => { view.dragging = true; const r = canvas.getBoundingClientRect(); view.x = e.clientX - r.left; view.y = e.clientY - r.top; }; window.addEventListener("mouseup", () => { view.dragging = false; }); canvas.onclick = (e) => { if (view.dragging) return; const r = canvas.getBoundingClientRect(), page = hit(e.clientX - r.left, e.clientY - r.top); if (page) { state.selectedIndex = page._index; render(true); } }; canvas.onwheel = (e) => { e.preventDefault(); const factor = e.deltaY < 0 ? 1.12 : .89; view.scale = Math.max(.45, Math.min(6, view.scale * factor)); draw(); };
